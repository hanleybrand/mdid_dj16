...
{% endif %}
    ...
    plugins: {
        audio: {
            url: "{{ server_url }}{% {% static lowplayer/flowplayer.audio-*.swf'|fileversion %}"
        },
        controls: {
            fullscreen: false,
            height: 30,
...
function insert_flowplayer() {
    ...
    $f("{{ anchor_id }}",
        "{{ server_url }}{% if flowplayer_key %}{% url{% static player/flowplayer.commercial-*.swf'|fileversion %}{% else %}{% url 'static' 'flowplayer/flowplayer-*.swf'|fileversion %}{% endif %}", clip).ipad();
}
